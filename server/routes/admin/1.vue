<template>
	<div class="xdapp-msg-wrapper">
		...
		<scroll  @tap.native='fnTap' :data="messages" :listen-scroll="listenScroll" :probe-type="probeType" :pulldown="pulldown" :pullup="true" @scrollToEnd="searchMore" @scrollToStart="refreshMore" class="scroll-container" ref="listview">
			<div class="msg-list">
				<ul class="list-group">
					<li class="list-group-item" v-for="(item,index) in messages" ref="groupItem">
						...
						<div :class="{'msg-item':item.poster==1,'msg-item-right':item.poster==2,'voice-no-arrow':!item.replyContent.includes('[###voice###]'),'img-no-arrow':item.replyContent.includes('[###img###]') }">
							<div v-if="item.poster==1">
								<img class="avatar-img" v-if="icons && icons.iconID && imgSrcOption" :src="imgSrcOption">
								<img class="avatar-img" v-else-if='icons && icons.gender==2' src="~@/public/asset/img/user/teacher-woman.png" alt="" />
								<img class="avatar-img" v-else-if='icons && (icons.gender==1 || icons.gender==0 || icons.gender==null)' src="~@/public/asset/img/user/teacher-man.png" alt="" />
								<img class="avatar-img" v-else src="~@/public/asset/img/user/teacher-man.png" alt="" />
							</div>
							
							<dd v-if="item.replyContent.includes('[###voice###]')" @click="playVoice(item,0,index)" class="reply audio-ico" >
                <div v-if="item.poster!=1" :style="{'marginLeft':JSON.parse( item.replyContent.replace('[###voice###]','') ).time+'px'}" class="dur">{{JSON.parse( item.replyContent.replace('[###voice###]','') ).time}}"</div>
                <div v-else :style="{'marginRight':JSON.parse( item.replyContent.replace('[###voice###]','') ).time+'px'}" class="dur">{{JSON.parse( item.replyContent.replace('[###voice###]','') ).time}}"</div>
                <div class="voice-img" :class="{speaking:cur==index&&isSpeaking}"></div>
              </dd>
              <div v-else-if="item.replyContent.includes('[###img###]')"  >
                   <!-- <div><img :id="JSON.parse( item.replyContent.replace('[###img###]','') ).id" data-preview-src="" data-preview-group="1" class="chat-img" src="http://pic41.nipic.com/20140524/9643307_104442624152_2.jpg" alt=""></div> -->
                   <div><img @load="_scrollToBottomAfterLoadImage" data-preview-src="" data-preview-group="1" class="chat-img" src="" :id="JSON.parse( item.replyContent.replace('[###img###]','') ).name.replace('.','-')" alt=""></div>
                <!-- <div class="dur">图片</div> -->
              </div>
              <dd v-else class="reply" v-html="item.replyContent"></dd>
...
<div v-show="showSpeakIcon" class="blackBoxSpeak">
    ...
    <p v-show="showCountdown" class="blackBoxSpeakConent"><span style="font-weight:bold">  {{countdown}} </span>秒后，自动发送</p>
</div>
...
<div v-show="showSpeakStop" class="blackBoxPause">
    <p class="blackBoxPauseContent" style="background: red">松开手指， 取消发送</p>
      ...
      <div class="audioButton" :class="{'btn-audio':toggle,'btn-text':!toggle}" @click="toggleAudioButton" style="margin-left: -10px;"></div>
      <div class="msg-left" v-show="showRecordButton" style="margin-left:0px;">
				<button class="recordButton" @touchstart.prevent="speakBegin" @touchend.prevent="speakEnd"  @touchmove.prevent="speaking" id="bt_recoding">{{audioBarText}}</button>
			</div>
      <!-- <div id="myyy" v-show="0" @click="speakEnd"></div> -->

			<div class="msg-left" v-show="!showRecordButton" style="margin-left:0px;">
				<textarea type="text" v-show="!sendingImage" class='input-text' v-model="content" @focus="setPosition()" @blur="setPosition2()"></textarea>
				<textarea type="text" v-show="sendingImage" class='input-text' ></textarea>
      </div>
			<div class="msg-right">
         <button @click="uploadHandler" class="btn-send btn-pic"></button>
				<button :disabled="content ? false : true" class="btn-send" :class="content ? 'btn-on' : 'btn-off'" @click="sendReply()">发送</button>
			</div>
		</div>
...
export default {
  data() {
    return {
      ...
      loading: false,
      params: {
        replyObject: this.$route.query.replyType,
        replyTime: null, //m2.date.addDays(m2.date.now() ,1),
        sort: PreOrLast.Pre,
  ...
  methods: {
                ...
                reader.onloadend = function (e) {
                    ...
                    upLoadFile(data, res => {
                      if(res[0]){
                        ...
                        _this.content=`[###video###]{"id":"${res[0].id}","time":"${_this.time2}","name":"${res[0].originalName}"}`; 
                        _this.sendReply();
                      }
    ...
    showImages(item,flag,i){
      if(typeof item.replyContent == 'string'){
        item = JSON.parse( item.replyContent.replace('[###img###]','') );
      }
      let r;
      // alert(item.id)

      plus.io.resolveLocalFileSystemURL('_doc/img/'+ item.name, (entry)=>{
        entry.file(async function(file) {
          ...
          fileReader.onloadend = function(evt) {
            var imgIcon = document.getElementById(item.name.replace('.','-'));
    ...
    getImage(item){
      this.task=null;
      this.task = plus.downloader.createDownload( `${downLoadFile}?MaterialID=${item.id}&OriginalName=${item.name}`, {method:"GET",filename:'_doc/img/'} ,(d,status)=>{
        // alert(d.filename)
        if ( status == 200 ) { 
          // alert(d.filename)
    ...
    initSpeakIcon(){
      //轮播相关
        ...
        // blackBoxSpeak.style.background = `url(${require('@/public/asset/img/message/ic_record@2x.png')}) no-repeat 28px 16px/64px 104px,url(${require('@/public/asset/img/message/speaking.gif')}) no-repeat 111.2px 32px/28.8px 88px`;
        //         blackBoxSpeak.style.backgroundColor = " rgba(0,0,0,.7)";
        // this.timer = setInterval( ()=> {
        //     setTimeout( () =>{
        //         this.num++;
        //         blackBoxSpeak.style.background = `url(${require('@/public/asset/img/message/ic_record@2x.png')}) no-repeat 28px 16px/64px 104px,url(${require('@/public/asset/img/message/ic_record_ripple@2x-'+ this.index[this.num] +'.png')}) no-repeat 111.2px 32px/28.8px 88px`;
        //         blackBoxSpeak.style.backgroundColor = " rgba(0,0,0,.7)";
        //     }, 70);
    ...
    playVoice(item,flag,i){
      ...
      if(typeof item.replyContent == 'string'){
        item = JSON.parse( item.replyContent.replace('[###voice###]','') );
      }

      plus.io.resolveLocalFileSystemURL('_doc/audio/'+ item.name, (entry)=>{
      ...
      // this.startPlay(item.replyContent)
    },
    getVoice(item){
      ...
      this.isSpeaking=true;
      this.task=null;
      this.task = plus.downloader.createDownload( `${downLoadFile}?MaterialID=${item.id}&OriginalName=${item.name}`, {method:"GET",filename:'_doc/audio/'} ,(d,status)=>{
        if ( status == 200 ) { 
          // mui.alert( "filename2222222: " + d.filename );
    ...
    checkInit(){
      ...
      if (this.replyType == ReplyType.Consultant || this.replyType == ReplyType.Educator) {
        if(this.pageTitle.indexOf('-')<0){
          if(this.replyType == ReplyType.Consultant){
            mui.confirm('该学员目前没有咨询师，请联系校区在系统中分配咨询师。', '提示', ['确定'], (e)=> {
          ...
          if(this.replyType == ReplyType.Educator){
            mui.confirm('该学员目前没有学管师，请联系校区在系统中分配学管师。', '提示', ['确定'], (e)=> {
              // this.$router.back()
              this.$router.push({path:'/message/customer-interact'})
            })
          }
...
.blackBoxSpeak {
		...
		bottom: 0;
		margin: auto;
		background: url(~@/public/asset/img/message/ic_record@2x.png) no-repeat 28px 16px/65px 104px,
		url(~@/public/asset/img/message/speaking.gif) no-repeat 111.2px 32px/28.8px 88px;
...
@keyframes voice{
  0% {background: url(~@/public/asset/img/message/voices001.png) no-repeat ; }
  25% {background: url(~@/public/asset/img/message/voices002.png) no-repeat ; }
  50% {background: url(~@/public/asset/img/message/voices003.png) no-repeat ; }
  75% {background: url(~@/public/asset/img/message/voices004.png) no-repeat ; }
  100% {background: url(~@/public/asset/img/message/voices004.png) no-repeat ; }
}
